...
// array의 요소 출력
// element1, element2, ....
let fruits = ['사과', '감', '귤', '배']; //배열은 큰 바구니임 순서대로 차곡차곡 넣어놓은 상태임 이거를 하나씩 다 꺼내고 싶어. 
                                        //for문으로 쓰면 //줄바꿈을 안하는 게 좋다
...
//위의 식을 i를 사과, 감, 귤, 배로 나열하지 않고 i = 0 을 이용한 식을 세우는 경우 (이하와 같다.)
let desk = ''; //책상위에 무언가를 ''; 이 빈칸에 올릴거에요 단, 단어나 문자일 경우에만 ''요고를 씁니다.
for (let i = 0; i <= fruits.length; i++) { // i <= 4 이렇게 하면 순서가 정해지지 않는다. 
    //그래서 fruits라는 배열에서.length (배열의 길이만큼)이라는 뜻을 가진 메소드 (함수)를 쓴다. 
   //for 문을 왜썻냐? = fruits라는 바구니에서 책상위에 과일을 하나씩 올릴거니까!! 컴퓨터는 2진수로 세팅된거라서 항상 count를 0에서부터 시작하니까 0썼고요.
    ...
    //index값이 어짜피 0에서부터 과일길이만큼 진행될거니까 그니까 어짜피 배열에서 하나씩 순서대로 꺼낼건데 
...
let table = '';
for (let k = 0; k <= 4; i++) {
    table += fruits[i];     //보통 배열은 [] 이것을 쓰기로 약속한거임. 그러니까 그냥 쓰면 됨!!!!! 
                            //우리끼리 약속한거야 임마!!!!!!! 쨔샤!!!!! ^_______^ 우리 약속 지켜줄꺼지?
}
